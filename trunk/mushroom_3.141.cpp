...
PointType EvalBoard(int depth, int OPlayer, int MPlayer, const axis &leftStone, const axis &rightStone, int type);
...
PointType MinValue(int depth, double alpha, double beta, int OPlayer, int MPlayer, axis leftStone, axis rightStone, axis lastStone)
{
	if (clock() - startTime > limitTime)
		return EvalBoard(depth, OPlayer, MPlayer, leftStone, rightStone, 0);
  ...
  node.stone.y = stone[0].y;
	...
	if (depth != MAXDEPTH)
	{
		...
		*/
		if (lastStone.x == 8 && lastStone.y == 7)
			TFind = TF;
  ...
  for(vector<axis>::const_iterator iter = stone.begin(); iter != stone.end(); iter++)
  {
    ...
    if (ret.val < node.val)
    {
      ...
      node.stone.y = iter->y;
		...
		/*
		else if (abs(ret.val - node.val) < eps && ret.depth > node.depth)
		{
			// depth越高且结果的得分相差不多时，采用depth高的
			node.stone.x = iter->x;
			node.stone.y = iter->y;
			node.depth = ret.depth;
		}
...
{
	if (clock() - startTime > limitTime)
		return EvalBoard(depth, OPlayer, MPlayer, leftStone, rightStone, 1);
...
bool checkLine(axis stone, int num, int xa, int ya, int xb, int yb)
{
	...
	{
		return (max(abs(aStone.x - bStone.x), abs(aStone.y - bStone.y)) >= 3 && 
				(pos(aStone.x + xa, aStone.y + ya) == EMPTY || pos(bStone.x + xb, bStone.y + yb) == EMPTY));
	}
}
